    ...
    if 'id' in session and 'session_id' in session:
        ...
        if check_token(token):
            try:
                connection = Getconnection.getconnection()
                cursor = connection.cursor()
                query = "SELECT * FROM user_answer ua LEFT JOIN Subject su ON ua.subjectname = su.id AND ua.user_id = %s"
                ...
                return jsonify({'status': 'error', 'message': 'Error retrieving test results from the database'})
        else:
            return jsonify({'status': 'error', 'message': 'Invalid token'})
    else:
        return jsonify({'status': 'error', 'message': 'User not logged in'})
...
@app.route('/get_testnumber', methods=['GET', 'POST'])
def get_testnumber():
    if 'id' in session and 'session_id' in session:
        user_id = session['id']
        token = session['session_id']
        ## Going forward add chapterid and subject id also in the execution query
        if check_token(token):
            try:
                connection = Getconnection.getconnection()
                cursor = connection.cursor()
                cursor.execute("SELECT max(test_no) from user_answer where user_id = %s", [user_id])
                testnumber = cursor.fetchone()
                print(testnumber)
                return jsonify({'status': 'success', 'testnumber': testnumber})
            except Exception as e:
                print(e)
